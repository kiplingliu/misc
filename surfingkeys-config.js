...
// - Slow
const REDLIB_URL = 'https://redlib.perennialte.ch';
...
function dispatchMouseClick(element, callback = api.Hints.dispatchMouseClick) {
    ...
    if (!element.hasAttribute('href') && element.querySelector('a[href]')) {
...
mapkey('yA', '#7Copy all links on page to clipboard', function () {
    const links = Array.from(document.querySelectorAll('a[href]'))
        ...
        // Filter out links that are just fragment identifiers on the current page
        .filter(href => !window.location.href.split('#')[0].startsWith(href.split('#')[0]))
        // Get unique links
